...
public class SQLDatosPer extends PoolConn  {
    ...
    public SQLDatosPer(String DataBase) throws SQLException, NamingException
    {
        ...
        try {
            ...
            PreparedStatement st = conn.prepareStatement("SELECT *,capitalsocial() from datosper where id=1");
            ...
            ResultSet rs = st.executeQuery();
                ...
                if (rs.next()) {
                    ...
                    this.fiscal_year=(StringUtils.isEmpty(rs.getString("fiscal_year"))) ? "": rs.getString("fiscal_year");
                    this.perido=(StringUtils.isEmpty(rs.getString("periodo").trim())) ? "": rs.getString("periodo");
                    
                    this.irpf_profesionales=rs.getBigDecimal("irpf_profesionales");
                    this.irpf_alquileres=rs.getBigDecimal("irpf_alquileres");
                    this.iva=rs.getBigDecimal("iva");
                    
                    this.forma_juridica=(StringUtils.isEmpty(rs.getString("forma_juridica"))) ? "": rs.getString("forma_juridica");                    
                    this.Sociedades=(StringUtils.isEmpty(rs.getString("sociedades"))) ? "": rs.getString("sociedades");
                    this.CriterioDeCaja=(StringUtils.isEmpty(rs.getString("criterio_de_caja"))) ? "": rs.getString("criterio_de_caja");
                    this.Presupuestos=(StringUtils.isEmpty(rs.getString("presupuestos"))) ? "": rs.getString("presupuestos");
    ...
    public byte[] getToken(String xUser) throws SQLException
    {
        ...
        try {
           ...
           rs.close();
            ...
            System.out.println("SELECT * from PersonalRRHH Connection Failed!.getToken");
            conn.close();
        }
        finally{
            conn.close();
    ...
    public void UpdateYearPeriodo(String xYear,String xPeriodo) throws SQLException
    {
        ...
        try {
            ...
            CallableStatement st = conn.prepareCall("{ call DatosPerYear(?,?) }");
            ...
            System.out.println("DatosPerYear Connection Failed!");
     ...
     * @throws SQLException 
     */
    public void CargaImpositiva(String xCriterio, String xFechaCons, String xCNAE,
            String xIRPFprof, String xIRPFalqui, String xIVA, byte[] x036, byte[] xEscrituras) throws SQLException
    {
        Connection conn = PGconectar();

        try {
            try (CallableStatement st = conn.prepareCall("{ call DatosPerCargaImpostiva(?,?,?,?,?,?,?,?) }")) {
                st.setString(1, xCriterio);
                st.setString(2, xFechaCons);
                st.setString(3, xCNAE);
                st.setString(4, xIRPFprof);
                st.setString(5, xIRPFalqui);
                st.setString(6, xIVA);
                st.setBytes(7, x036);
                st.setBytes(8, xEscrituras);
            ...
            System.out.println("DatosPerCargaImpostiva Connection Failed!");
     ...
     */
    public void nextRecibida()  throws SQLException, NamingException
    {
        ...
        try {
            ...
            st = conn.prepareStatement("select nextval('cont_recibidas')");
